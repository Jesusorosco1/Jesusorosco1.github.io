...
export function Content() {
  // Parche para evitar errores de tipos en Vercel (placeholder, capture events, resize)
  const fixProps = {
    placeholder: "",
    ...
    <section className="py-12 px-8 bg-[#0f1115] text-white">
      <div className="">
        ...
        <div className="flex gap-4 overflow-x-auto pb-10 scrollbar-hide">
          {MODELS.map((car) => (
            <Link href="/catalogo" key={car.name}>
              <Card 
                ...
                {...fixProps}
              >
                <CardBody className="p-5" {...fixProps}>
        ...
        <div className="mt-4 flex flex-col md:flex-row gap-4">
          <Link href="/catalogo" className="w-full md:w-auto">
            <Button 
              variant="outlined" 
              color="white" 
              className="rounded-full px-8 py-3 text-xs normal-case border-gray-700 hover:bg-white/5 w-full"
              {...fixProps}
            >
              Ver modelos
            </Button>
