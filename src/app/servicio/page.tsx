    ...
    <main className="min-h-screen bg-white">
      ...
      <section className="py-16 px-8">
        <div className="container mx-auto max-w-5xl text-center">
          <Typography className="mb-8 text-gray-700 leading-relaxed" {...fixProps}>
          ...
          <Typography className="mb-12 text-gray-700 font-bold uppercase tracking-wider" {...fixProps}>
            ¡Estamos aquí para servirle!
          </Typography>
          
          <div className="bg-gray-50 p-8 rounded-xl inline-block w-full md:w-auto">
            <Typography variant="h6" className="mb-4 text-gray-900 uppercase" {...fixProps}>
              ¿TE INTERESA: Las 5 mejores camionetas para la familia?
            </Typography>
            <Button size="lg" className="bg-[#00adef] rounded-none px-12 py-4 text-sm" {...fixProps}>
      ...
      <section className="py-16 px-8 border-t border-gray-100">
        <div className="container mx-auto grid grid-cols-1 lg:grid-cols-2 gap-20">
          ...
          {/* Columna JAC SERVICE */}
          <div>
            <Typography variant="h2" className="mb-6 text-4xl font-bold text-[#00adef] uppercase tracking-tighter" {...fixProps}>
              JAC SERVICE
            </Typography>
            <Typography className="mb-8 text-gray-600 text-sm leading-relaxed" {...fixProps}>
            ...
            <div className="space-y-2">
              {ESTADOS.slice(0, 5).map((estado, idx) => (
                <Accordion 
                  key={estado} 
                  open={openService === idx + 1} 
                  icon={<Icon id={idx + 1} open={openService} />}
                  {...fixProps}
                >
                  <AccordionHeader 
                    onClick={() => handleOpenService(idx + 1)} 
                    className="text-sm font-medium uppercase py-3 border-b-gray-300"
                    {...fixProps}
                  >
                    {estado}
                  </AccordionHeader>
                  <AccordionBody className="text-xs text-gray-500" {...fixProps}>
          ...
          <div>
            ...
            <div className="space-y-2">
              {ESTADOS.map((estado, idx) => (
                <Accordion 
                  key={estado} 
                  open={openSpare === idx + 1} 
                  icon={<Icon id={idx + 1} open={openSpare} />}
                  {...fixProps}
                >
                  <AccordionHeader 
                    onClick={() => handleOpenSpare(idx + 1)} 
                    className="text-sm font-medium uppercase py-3 border-b-gray-300"
                    {...fixProps}
                  >
                    {estado}
